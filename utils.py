...
def replace_module(model, replacement_dict={}, exception_dict={}, arch="presnet18"):
    ...
    """ 
    assert arch in ["densenet", "presnet32", "presnet32-standard","presnet20", "presnet20-standard", "presnet18", "glouncv-alexnet", "glouncv-alexnet-bn",  "postech-alexnet", "glouncv-presnet34", "glouncv-presnet50", "glouncv-mobilenetv2_w1"],\
            ("Not support this type of architecture !")

    model = replace_all(model, replacement_dict=replacement_dict)

    if arch == "presnet32" or arch == "presnet32-standard" or arch == "presnet20" or arch == "presnet20-standard":
...
def add_weight_decay(model, weight_decay, skip_keys, verbose=False):
    decay, no_decay = [], []
    for name, param in model.named_parameters():
        ...
        for skip_key in skip_keys:
            if skip_key in name:
                if verbose: print ("Skip weight decay for: ", name)
